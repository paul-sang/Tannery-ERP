      ...
      @for (toast of toastService.toasts(); track toast.id) {
        <div 
          ...
          <div class="flex-1 w-0">
            <h3 class="text-sm font-semibold text-slate-800">{{ toast.title }}</h3>
            <p class="mt-1 text-sm text-slate-600">{{ toast.message }}</p>
          </div>

          <!-- Close Button -->
          <div class="ml-4 flex-shrink-0 flex">
            <button (click)="toastService.remove(toast.id)" class="rounded-md inline-flex text-slate-400 hover:text-slate-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500 transition-colors cursor-pointer">
...
export class ToastContainerComponent {
  ...
  getToastClasses(toast: ToastMessage): string {
    switch (toast.type) {
      case 'success': return 'bg-white border-emerald-200 border-l-4 border-l-emerald-500';
      case 'error': return 'bg-white border-red-200 border-l-4 border-l-red-500';
      case 'warning': return 'bg-white border-amber-200 border-l-4 border-l-amber-500';
      default: return 'bg-white border-blue-200 border-l-4 border-l-blue-500';
    }
  }
}
